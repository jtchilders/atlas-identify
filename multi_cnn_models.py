...
def build_model(config,args,print_summary=True):
   ...
   for i in range(0,image_shape[0],4):
      ...
      logger.debug('i = %s',i)
      output = kl.Lambda(lambda x: x[:,:,i:i+4,:,:])(input_image)
      logger.debug('i = %s',output)
      ...
      output = kl.Conv3D(1024,(3,3,3),strides=(1,1,1),padding='same',name='conv3d_{0}'.format(layer_num),use_bias=False,data_format='channels_first')(output)
      output = kl.Activation('relu',name='relu_{0}'.format(layer_num))(output)
      output = kl.MaxPooling3D(pool_size=(1,2,2), data_format='channels_first',name='pool_{0}'.format(layer_num))(output)
      output = kl.Dropout(0.25,name='dropout_{0}'.format(layer_num))(output)
      layer_num += 1
   ...
   output = kl.Concatenate(name='concat_{0}'.format(layer_num))(outputs)


   output = kl.Conv3D(512,(3,3,3),strides=(1,1,1),padding='same',name='conv3d_{0}'.format(layer_num),use_bias=False,data_format='channels_first')(input_image)
   output = kl.Activation('relu',name='relu_{0}'.format(layer_num))(output)
   output = kl.MaxPooling3D(pool_size=(2,2,2), data_format='channels_first',name='pool_{0}'.format(layer_num))(output)
   output = kl.Dropout(0.25,name='dropout_{0}'.format(layer_num))(output)
   ...
   layer_num += 1
   output = kl.Conv3D(246,(3,3,3),strides=(1,1,1),padding='same',name='conv3d_{0}'.format(layer_num),use_bias=False,data_format='channels_first')(output)
   output = kl.Activation('relu',name='relu_{0}'.format(layer_num))(output)
   output = kl.MaxPooling3D(pool_size=(1,2,2), data_format='channels_first',name='pool_{0}'.format(layer_num))(output)
   output = kl.Dropout(0.25,name='dropout_{0}'.format(layer_num))(output)
